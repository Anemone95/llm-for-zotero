...
describe("agentTrace render", function () {
  ...
  it("summarizes file_io aliases and malformed actions without false write labels", function () {
    const events: AgentRunEventRecord[] = [
      ...
      },
      {
        ...
        payload: {
          ...
          args: {
            operation: "read_file",
            file_path: "/tmp/local-section.md",
            offset: 64,
          },
        },
        createdAt: 3,
    ...
    assert.include(rowTexts, "Reading local-paper.md");
    assert.include(rowTexts, "Reading local-manifest.json");
    assert.include(rowTexts, "Reading local-section.md");
    assert.include(rowTexts, "Accessing local-paper.md");
    assert.notInclude(rowTexts, "Writing local-paper.md");
    assert.include(codeBlocks, "read /tmp/local-manifest.json");
    assert.include(codeBlocks, "read_file /tmp/local-section.md");
  });
