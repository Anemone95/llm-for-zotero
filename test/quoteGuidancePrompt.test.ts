...
describe("quote guidance prompts", function () {
  ...
  it("guides figure tasks through paper_read visual and targeted modes", async function () {
    ...
    assert.include(text, "mode:'visual'");
    assert.include(text, "mode:'targeted'");
    assert.notInclude(text, "MinerU");
    assert.notInclude(text, "full.md");
  });
  ...
  it("describes figure image support generically without naming specific models", function () {
    ...
    assert.include(text, "Visual models see images directly");
