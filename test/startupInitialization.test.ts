...
describe("plugin startup initialization", function () {
  ...
  it("keeps optional runtime services deferred without blocking startup", function () {
    ...
    assert.notInclude(source, "scheduleWebChatRelayRegistration();");
    assert.notInclude(source, "scheduleMineruAutoWatchRegistration();");
  });
});

describe("legacy startup migrations", function () {
  it("keeps preference migrations without deferred MinerU cache migrations", function () {
    ...
    assert.notInclude(deferredBody, "migrateMineruContentMdCleanup");
    assert.notInclude(deferredBody, "migrateMineruManifestBuild");
  });
});
