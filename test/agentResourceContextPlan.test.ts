...
describe("agent resource context plan", function () {
  ...
  it("describes selected screenshots without inlining image inputs", async function () {
    const messages = await buildAgentInitialMessages(
      request({
        model: "gpt-4.1",
    ...
    assert.isString(userMessage.content);
    assert.include(
      String(userMessage.content),
      "agent mode does not inline-send images",
    );
  });

  it("omits image and PDF content inputs for image-only content inputs", async function () {
    const messages = await buildAgentInitialMessages(
      request({
    ...
    assert.notInclude(text, "image_url");
    assert.notInclude(text, "file_ref");
    assert.include(text, "/tmp/paper.pdf");
  });

  it("omits image and PDF content inputs for PDF-document content inputs", async function () {
    const messages = await buildAgentInitialMessages(
      request({
