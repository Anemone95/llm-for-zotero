...
describe("composeContextController paper card attachment labels", function () {
  ...
  it("shows the live attachment title for PDF cards and tooltips", function () {
    zoteroItems.set(
      101,
      makeAttachment({
        id: 101,
    ...
    assert.equal(
      formatPaperContextCardAttachmentLine(paperContext, "pdf"),
      "Supplementary Material",
    );
    const tooltip = formatPaperContextChipTitle(paperContext, "pdf");
  ...
  it("keeps a single selected HTML attachment available as an attachment card", function () {
    const paperContext = makePaperContext({
      contextItemId: 101,
      attachmentTitle: "snapshot.html",
    });
  ...
  it("falls back to stored context data, then an empty label, when lookup fails", function () {
    const storedContext = makePaperContext({
      contextItemId: 404,
      attachmentTitle: "Stored Supplement Title",
    });
    assert.equal(
      formatPaperContextCardAttachmentLine(storedContext, "pdf"),
      "Stored Supplement Title",
    );

    const missingContext = makePaperContext({ contextItemId: 405 });
    assert.equal(
      formatPaperContextCardAttachmentLine(missingContext, "pdf"),
      "",
    );
  });
});
