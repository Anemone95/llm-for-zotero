...
export type ReferenceSelectorTagScope = "all" | "allTagged" | "untagged";
export type ReferenceSelectorTagMatchMode = "and" | "or";

export type ReferenceSelectorTagInfo = {
  name: string;
...
export type ReferenceSelectorTagAvailability = {
  available: boolean;
  count: number;
};

type ReferenceSelectorTagIndex = Map<string, ReferenceSelectorTagInfo>;

function normalizeReferenceSelectorTagName(value: unknown): string {
  return `${value ?? ""}`.replace(/\s+/g, " ").trim();
}

function buildReferenceSelectorTagIndex(
  items: readonly ReferenceSelectorTagIndexItem[],
  options: {
    includeAutomatic?: boolean;
    getColor?: (name: string) => string | null;
  } = {},
): ReferenceSelectorTagIndex {
  const index: ReferenceSelectorTagIndex = new Map();
  const add = (tagName: string, automatic: boolean) => {
    const name = normalizeReferenceSelectorTagName(tagName);
    if (!name) return;
    ...
    const existing =
      ...
      } satisfies ReferenceSelectorTagInfo);
    existing.count += 1;
    if (automatic) existing.automaticCount += 1;
    else existing.manualCount += 1;
    index.set(normalizedName, existing);
  };
  for (const item of items) {
    for (const tag of item.tags) add(tag, false);
    if (options.includeAutomatic) {
      for (const tag of item.tagsAuto) add(tag, true);
    }
  }
  return index;
}

function getSortedReferenceSelectorTagInfos(
  index: ReferenceSelectorTagIndex,
): ReferenceSelectorTagInfo[] {
...
function referenceSelectorItemMatchesTagFilter(
  item: ReferenceSelectorTagIndexItem,
  selectedTags: ReadonlySet<string>,
  includeAutomatic: boolean,
  matchMode: ReferenceSelectorTagMatchMode,
): boolean {
  if (!selectedTags.size) return true;
  const itemTags = new Set<string>();
  for (const tag of item.tags) {
    const normalized = normalizeReferenceSelectorTagIdentityName(tag);
    if (normalized) itemTags.add(normalized);
  }
  if (includeAutomatic) {
  ...
  const matches = [...selectedTags].filter((tag) => itemTags.has(tag));
  return matchMode === "or"
    ? matches.length > 0
    : matches.length === selectedTags.size;
}

function filterReferenceSelectorItemsForTagView(
  items: readonly ReferenceSelectorTagIndexItem[],
  options: {
    scope?: ReferenceSelectorTagScope;
  ...
  const selectedTags = new Set(
    [...(options.selectedTags || [])]
      .map(normalizeReferenceSelectorTagIdentityName)
      .filter(Boolean),
  );
  const matchMode = options.matchMode || "and";
  return items.filter((item) => {
    const manualCount = item.tags.filter((tag) =>
      normalizeReferenceSelectorTagIdentityName(tag),
    ).length;
    const automaticCount = includeAutomatic
      ? item.tagsAuto.filter((tag) =>
          normalizeReferenceSelectorTagIdentityName(tag),
        ).length
      : 0;
    const tagCount = manualCount + automaticCount;
    if (options.scope === "allTagged" && tagCount === 0) return false;
    if (options.scope === "untagged" && tagCount > 0) return false;
    return referenceSelectorItemMatchesTagFilter(
      item,
      selectedTags,
      includeAutomatic,
      matchMode,
...
function computeReferenceSelectorTagAvailability(
  tagInfos: readonly ReferenceSelectorTagInfo[],
  items: readonly ReferenceSelectorTagIndexItem[],
  selectedTags: ReadonlySet<string>,
  includeAutomatic: boolean,
): Map<string, ReferenceSelectorTagAvailability> {
  const availability = new Map<string, ReferenceSelectorTagAvailability>();
  for (const info of tagInfos) {
    const selectedWithTag = new Set(selectedTags);
    selectedWithTag.add(info.normalizedName);
    const count = filterReferenceSelectorItemsForTagView(items, {
      selectedTags: selectedWithTag,
      includeAutomatic,
      matchMode: "and",
    }).length;
    availability.set(info.normalizedName, {
      available: count > 0,
      count,
    });
  }
...
export type ReferenceSelectorState = {
  ...
  tagScope: ReferenceSelectorTagScope;
  selectedTags: Set<string>;
  tagFilterQuery: string;
  tagMatchMode: ReferenceSelectorTagMatchMode;
...
export type ReferenceSelectorTagViewModel = {
  ...
  allTagInfos: ReferenceSelectorTagInfo[];
  filteredTagInfos: ReferenceSelectorTagInfo[];
  availability: Map<string, ReferenceSelectorTagAvailability>;
...
export function filterReferenceSelectorGroupsForTagView(
  ...
  return filterReferenceSelectorItemsForTagView(
...
export function buildReferenceSelectorTagViewModel(
  ...
  const baseTagIndex = buildReferenceSelectorTagIndex(baseItems, {
    includeAutomatic: state.showAutomaticTags,
    getColor,
  });
  const allTagInfos = getSortedReferenceSelectorTagInfos(baseTagIndex);
  ...
  const availability = computeReferenceSelectorTagAvailability(
    allTagInfos,
    availabilityItems,
    state.selectedTags,
...
export function normalizeReferenceSelectorTagIdentityName(value: unknown): string {
  const name = normalizeReferenceSelectorTagName(value);
